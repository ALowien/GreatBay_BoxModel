...
precip <- precipitation %>%
   select(UNH_ID:pH, NH4_UGL = NH4, NO3_MGL = Nitrate, PO4_UGL = PO4_Final, TDN_MGL = TDN, DOC_MGL = NPOC,
         DON_MGL = DON, Field.Notes, Lab_Notes, TDNf:cbf) %>%
...
precip$DIN_MGL <- precip$NO3_MGL + (precip$NH4_UGL/1000)

#Remove flagging columns now
precip <- precip %>%
  select(UNH_ID:Project, Rainfall_mm, pH:Lab_Notes, DON_MGL_Final_MDL, DIN_MGL, - DON_MGL)
...
cy_rainfall <- read_excel("data/precipitation/TF2_CalYr_WaterYr_precipsum.xlsx", sheet =3) %>%
  filter(CY >2007 & CY < 2019)

#Plot Rainfall
p_rainfall_cy <- ggplot(cy_rainfall, aes(CY, Rainfall_mm)) + geom_point() + 
  scale_x_continuous(breaks=seq(from=2008,to=2018,by=1)) + 
  scale_y_continuous(limits=c(0,1800), breaks=seq(from=0,to=1800, by=200))+ theme_cowplot()
p_rainfall_cy 
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#WY: Loads table
precip_wy_loads_final <- precip_wy_loads %>%
  select(WY, Rainfall_mm, TDN_kg_ha_yr:DON_kg_ha_yr)
...
#Add in Rainfall
monthly_precip_mm <- read_excel("data/precipitation/Monthly_Precipitation.xlsx")

monthly_precip_mm<- monthly_precip_mm %>%
  select(Month_Year, CY=Year, Month, Rainfall_mm)
...
write.csv(precip_cy_loads_final, "results/main_precipitation_format/cy_precip_loads.csv")
write.csv(precip_wy_loads_final, "results/main_precipitation_format/wy_precip_loads.csv")
write.csv(precip_month_loads_final, "results/main_precipitation_format/month_precip_loads.csv")
